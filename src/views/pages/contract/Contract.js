...
const Sekretariat = () => {
  ...
  const GetContracts = () => {
    setIsLoading(true)
    const url = `http://192.168.88.58:8080/contracts?company=${selectedCompany.value}&page=${currentPage}&length=${itemsPerPage}`
  ...
  const SearchContract = () => {
    setIsLoading(true)
    const url = `http://192.168.88.58:8080/contracts?keyword=${inputSearch}&company=${selectedCompany.value}&page=${currentPage}&length=${itemsPerPage}`
